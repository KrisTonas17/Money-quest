    ...
    <div className="bg-slate-800 rounded-2xl p-5 space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        {ALL_ACHIEVEMENTS.map((a) => {
          ...
          return (
            <div key={a.id} className={`rounded-xl p-3 ${unlocked ? "bg-gold/20 border border-gold" : "bg-slate-700 opacity-50"}`}>
              <div className="text-2xl">{a.emoji}</div>
              <p className={`text-xs font-semibold mt-1 ${unlocked ? "text-gold" : "text-slate-400"}`}>{a.label}</p>
...
Now the lesson JSON files. I'll write all 8:
