...
function SessionContent() {
  ...
  return (
    <main className="min-h-screen px-4 py-6 max-w-lg mx-auto">
      ...
      {phase === "story" && (
        <StoryScene content={sessionContent} onContinue={() => setPhase("decision")} />
      )}
      {phase === "decision" && (
        <DecisionCard card={sessionContent.decisionCard} gameState={gameState} onChoose={handleDecision} />
      )}
      {phase === "minigame" && (
        <MiniGame type={sessionContent.miniGameType} modifiers={sessionContent.miniGameModifiers} seed={seed} onComplete={handleMiniGameComplete} />
      )}
      {phase === "results" && (
        <SessionResults content={sessionContent} choiceIndex={chosenCard} miniGameScore={miniGameScore} gameState={gameState} onNext={handleNextSession} />
      )}
    </main>
  );
}
